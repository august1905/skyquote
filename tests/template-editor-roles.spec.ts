...
test.describe('Recipients / Roles panel', () => {
	test('adds, edits, reorders, and removes roles, and it persists through a reload', async ({ page }) => {
		...
		// Reorder: move "Sales Rep" (row 3) up above "Client".
		await rows.nth(3).getByRole('button', { name: 'Move role up' }).click();
		await expect(rows.nth(2).getByLabel('Role name')).toHaveValue('Sales Rep');
		await expect(rows.nth(3).getByLabel('Role name')).toHaveValue('Client');
		...
		await expect(reloadedRows.nth(2).getByLabel('Role name')).toHaveValue('Sales Rep');
		await expect(reloadedRows.nth(2).getByLabel('Sender')).toBeChecked();
		await expect(reloadedRows.nth(3).getByLabel('Role name')).toHaveValue('Client');
		await expect(reloadedRows.nth(3).getByLabel('Signing order')).toHaveValue('2');
		await expect(reloadedRows.nth(3).getByLabel(/color$/)).toHaveValue('#ff0000');

		await reloadedRows.nth(3).getByRole('button', { name: /Remove/ }).click();
		await expect(page.locator('.roles-panel-row')).toHaveCount(3);
		await expect(reloadedRows.nth(2).getByLabel('Role name')).toHaveValue('Sales Rep');

		// Removing the rest — Sales Rep, then both seeded roles — restores the
